...
class SCSGNet(nn.Module):
    def __init__(self, channels=32, output_stride=32, pretrained=True):
        super(SCSGNet, self).__init__()
        self.resnet = res2net50_v1b_26w_4s(pretrained=pretrained, output_stride=output_stride)

        self.context2 = SPFF(512, channels)
        self.context3 = SPFF(1024, channels)
        self.context4 = SPFF(2048, channels)

        self.decoder = PPD(channels)

        self.nonlocal2 = DLCC(512)
        self.nonlocal3 = DLCC(1024)
        self.nonlocal4 = DLCC(2048)

        self.attention2 = TAG(512, 64, 2, 3)
        self.attention3 = TAG(1024, 64, 2, 3)
        self.attention4 = TAG(2048, 256, 3, 5)
    ...
    def forward(self, sample):
        ...
        x3 = self.nonlocal3(x3)
        x4 = self.nonlocal4(x4)
